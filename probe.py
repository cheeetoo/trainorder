...
def get_activations_batched(model, prompts, hook_point, batch_size):
    """Run model with cache in batches to avoid OOM."""
    all_acts = []
...
with torch.no_grad():
    first_acts = get_activations_batched(model, first_prompts, cfg.hook_point, cfg.probe_batch_size)
    last_acts = get_activations_batched(model, last_prompts, cfg.hook_point, cfg.probe_batch_size)
